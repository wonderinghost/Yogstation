...
/obj/item/pipe_dispenser
	...
	///Speed of building plumbing pipes
	var/plumbing_build_speed = 2 DECISECONDS
	///speed of destroying pipes
...
/obj/item/pipe_dispenser/pre_attack(atom/atom_to_attack, mob/user, params)
	...
	// yogs start - disposable check
		if(istype(attack_target, /obj/item/pipe))
	...
	// yogs end
		...
		if(do_after(user, destroy_speed, attack_target))
			activate()
			qdel(attack_target)
		return
		...
		switch(category) //if we've gotten this var, the target is valid
			if(ATMOS_CATEGORY) //Making pipes
				...
				if (recipe.type == /datum/pipe_info/meter)
					...
					if(do_after(user, atmos_build_speed, attack_target))
						activate()
			...
			if(DISPOSALS_CATEGORY) //Making disposals pipes
				if(!can_make_pipe)
					return ..()
				var/turf/attempting_turf = get_turf(attack_target)
				...
				playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
				if(do_after(user, disposal_build_speed, attack_target))
			...
			if(TRANSIT_CATEGORY) //Making transit tubes
				...
				playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
				if(do_after(user, transit_build_speed, attack_target))
					activate()
			...
			if(PLUMBING_CATEGORY) //Making pancakes
				...
				playsound(get_turf(src), 'sound/machines/click.ogg', 50, 1)
				if(do_after(user, plumbing_build_speed, attack_target))
